                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap dark:text-gray-400 text-blue-900 font-bold">
                                <a href="{{route('levels.show',$schoolLevel->id)}}">
                                    {{ $schoolLevel->name }}
                                </a>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium dark:text-gray-400">
                                <a href="{{ route('levels.edit', $schoolLevel->id) }}"
                                    class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                <!-- Inside your table in Blade view -->
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form id="deleteForm-{{ $schoolLevel->id }}"
                                    action="{{ route('levels.destroy', $schoolLevel->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>

                                <button onclick="confirmDelete('{{ $schoolLevel->id }}')"
